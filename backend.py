...
"""
################################################################################
import os, sys, time, logging, configpi, re, warnings, json
import six
if six.PY3:
...
class Backend():
    ...
    def network_info(self):
        ...
        for id_node, node in self.network.nodes.items():
            net_infos[str(id_node)] = {"Is Ready":node.is_ready, "Node name":node.name ,"Node type":node.type, 
            "Product name":node.product_name, "Neighbours":list(node.neighbors), "Query Stage":node.query_stage}

        net_infos["Network Home ID"] = self.network.home_id_str
    ...
    def get_nodes_list(self):
        ...
        """
        d = OrderedDict()
        for k in self._my_nodes().values():
                if k.is_ready:
                    d[k.node_id] = k.product_name
                else:
                    d[k.node_id] = "[not ready]"
        return d
    ...
    def add_node(self):
        ...
        """
        if not self.network.is_ready:
            raise RuntimeError("Network isn't ready !")

        if self.network.controller.add_node():
            start = time.time()
            while self.node_added == None and (time.time() - start) < 20:
                time.sleep(1)
            
            if self.node_added != None:
                n = self.node_added
                self.node_added = None
                return json_prepare(n)
            else:
    ...
    def remove_node(self):
        ...
        if self.network.controller.remove_node():
            start = time.time()
            while self.node_removed == None and (time.time() - start) < 20:
                time.sleep(1)
    ...
    def set_node_parameter(self, n, pindex, value, size):
        ...
        if not node.is_ready:
            raise RuntimeError("Node isn't ready")
    ...
    def get_node_parameter(self, n, pindex):
        ...
        default_value = ""
        for k,v in node.get_configs().items():
            if v.index == pindex:
                    default_value = v.data

        if not default_value:
                raise RuntimeError("pindex is out of bound")
...
class Backend_with_sensors(Backend):
    ...
    def get_sensor_humidity(self, n):
        ...
        value = [
            v.data for k,v in values.items()
        ][0]
    ...
    def get_sensor_motion(self, n):
        ...
        # <http://www.openzwave.com/dev/classOpenZWave_1_1SensorMultilevel.html>
        values = node.get_values(
            class_id=0x30,      # COMMAND_CLASS_SENSOR_MULTILEVEL
            genre="User", readonly=True, writeonly=False,
    ...
    def get_sensor_battery(self, n):
        ...
        # <http://www.openzwave.com/dev/classOpenZWave_1_1SensorMultilevel.html>
        values = node.get_values(
            class_id=0x80,      # COMMAND_CLASS_SENSOR_MULTILEVEL
            genre="User", readonly=True, writeonly=False,
    ...
    def get_sensor_readings(self, n):
        ...
        values = {}
        values["battery"] = self.get_sensor_battery(n)["value"] 
        values["controller"] = self.controller_name
        values["humidity"] = self.get_sensor_humidity(n)["value"]
        values["location"] = node.location
        values["luminance"] = self.get_sensor_luminance(n)["value"] 
        values["motion"] = self.get_sensor_motion(n)["value"] 
        values["sensor"] = node.node_id 
        values["temperature"] = self.get_sensor_temperature(n)["value"]
        values["updateTime"] = self._get_node_timestamp(node)
    ...
    def set_sensors_parameter(self, index, value, size):
        ...
        result = {}
        for k,v in self.get_sensors_list().items():
            node = self._lookup_sensor_node(k)
            if not node.is_ready:
                result[node.node_id] = [False, "Node isn't ready"]
                continue

            if self.set_node_parameter(node.node_id, index, value, size):
                result[node.node_id] = [True, "OK"]
            else:
                result[node.node_id] = [False, "Command failed"]
...
class Backend_with_dimmers(Backend):
    ...
    def get_dimmer_level(self, n):
        ...
        values = node.get_values(
            class_id=0x26,      # COMMAND_CLASS_SENSOR_MULTILEVEL
            genre="User", readonly=False, writeonly=False,
            label=self._labels_xref['level']
        )

        if not values:
            raise RuntimeError("level: label not found. Is this a dimmer?")
    ...
    def set_dimmer_level(self, n, value):
        ...
        value_id = [
            v.value_id for k,v in values.items()
        ][0]

        old_value = node.get_dimmer_level(value_id)
